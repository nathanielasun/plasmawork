...
**Last updated: 2026-05-07 (Phase 0.5 Layers 1, 2, 3, and most of 4 structurally complete. Layer-4 progress: L4.12 (health/readiness/metrics with Prometheus text + injectable MetricsRegistry), L4.1 (workspace + members CRUD with atomic membership transitions and SHA-256-prefix-redacted target user ids in audit metadata), L4.2 (capsule CRUD + fork with If-Match-driven VERSION_CONFLICT and server-derived fork source), L4.4 (tool CRUD + promote-request with workspace-owned UNION global-trusted listing per §10.3 and route-level refusal of `validated`/`trusted` PATCH transitions), L4.6 (approval-request create/approve/deny with action-bound L2.9 high-risk gate; consume happens via `requireApprovalIfHighRisk` middleware), L4.7 (audit-events + provenance-events read with audit-read-pool gating, keyset cursor pagination, defense-in-depth metadata redaction on every output row, and SELECT lists that omit chain-internal fields). All Layer-4 routes use `composeMiddleware` for §6.2 ordering, Ajv with `additionalProperties: false` for v4 §4.1 enforcement, UUIDv4 path-param assertions, and never read actor identity from `req.body`. secure_core suite: 553 tests pass + 87 skipped across 45 files. Convention checker at 1137 checks. Remaining Layer-4 work: L4.3 (runs CRUD + cancel), L4.5 (artifacts + export), L4.8 (recovery flows), L4.9 (bootstrap), L4.10 (operator), L4.11 (worker token issuance — L3.9 upload route is already shipped). Layer-5 §29 live-runtime CI lane and FastAPI cut-over remain future work.)**
