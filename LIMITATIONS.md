...
**Last updated: 2026-05-06 (Phase 0.5 Layer-1 primitives are implemented, the five Layer-0 ADRs are accepted, and Layer-2 middleware (auth, CSRF, schema validation, workspace loading + uniform-404, membership, capability gating, object-scope, audit-actor) is implemented and tested. Sandbox runtime service, approval-required gating, archive-extraction safety, rate limiting, route plugins, and the cut-over from the single-user FastAPI workbench are still future work.)**
