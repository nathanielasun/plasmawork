...
**Last updated: 2026-05-06 (Phase 0.5 Layer-1 primitives are implemented, the five Layer-0 ADRs are accepted, and Layer-2 middleware (auth, CSRF, schema validation, workspace loading + uniform-404, membership, capability gating, object-scope, audit-actor) plus the L2.10 workspace path builder + safe-open and L2.11 archive extraction safety (zip + tar with §9.4 component validation, fail-closed size + count caps, and `archive.entry_rejected` audit) are implemented and tested. Sandbox runtime service, approval-required gating, rate limiting, route plugins, and the cut-over from the single-user FastAPI workbench are still future work.)**
