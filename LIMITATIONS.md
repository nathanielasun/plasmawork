...
**Last updated: 2026-05-05 (Phase 0.5 Layer-0 in-flight — all three pre-implementation gates partially landed. G0: ten v4 residual fixes (R1–R10) applied to `secure_multi_user_scaffolding_plan_v4.md` (archive size + file-count limits, csrf/origin audit events, unauthenticated actor_type, approval:request + run:approve_hpc capabilities, storage_reservations lifecycle, period_end CHECK, operator_events FK NOT NULL, security-configuration enumeration, tested JCS library mandate). G1: five Layer-0 ADRs drafted as Proposed (ADR-0008 secure-core language/layout: TS+Fastify+Drizzle+Postgres in `packages/secure_core/`; ADR-0009 sandbox: gVisor + L7 proxy; ADR-0010 WORM: S3 Object Lock Compliance Mode; ADR-0011 secrets: AWS Secrets Manager; ADR-0012 worker uploads: Option A server-mediated). G2: `packages/secure_core/IMPLEMENTATION_MANIFEST.md` ships project layout, error envelope, fixture conventions, per-endpoint canonical recipe. Awaiting human review to flip ADRs to Accepted, then Layer 1 unblocks.)**
