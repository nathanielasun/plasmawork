...
**Last updated: 2026-05-07 (Phase 0.5 Layer-1 primitives + the five Layer-0 ADRs + Layer-2 (11 of 12 middleware modules including rate-limit, workspace path safety, and archive extraction) are implemented and tested. Layer-3 group A+B services (audit chain DB writer + verifier with tail-truncation detection; approval system with v4 §16.4 atomic consumption + §16.3 token_context_hash; capsule version + lock with VERSION_CONFLICT semantics; quota counters + storage reservations with §21.2 atomic conditional UPDATE and §21.3 expiration sweep; run state machine with the full v4 §14 ten-state graph) all shipped via parallel sub-agents. L3.2 (external WORM anchor committer), L3.7–L3.10 (sandbox runner, worker token issuer, worker upload endpoint, SSRF-safe fetcher), L2.9 (requireApprovalIfHighRisk middleware now unblocked by L3.3), route plugins, and the FastAPI cut-over remain future work.)**
