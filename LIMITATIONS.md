...
**Last updated: 2026-05-05 (Phase 0.5 Layer-0 drafted, NOT complete — Layer 1 remains blocked. Per `program_development/phase_05_security_implementation_plan.md`, the gate requires all five Layer-0 ADRs at status `Accepted`; ADR-0008..ADR-0012 are at status `Proposed` and only the human owner may flip them. Round-2 review fixes landed: ADR numbering conflict resolved (final-form ADR moved from the already-taken ADR-0004 slot to ADR-0013), ADR-0011 + ADR-0012 corrected to Shape-A TypeScript paths (Python references replaced with `packages/secure_core/src/...`), ADR-0009 networking topology committed to UNIX-domain socket only (no veth, no `--network=none` contradiction), implementation plan §29 mapping refreshed to cover tests #1–#84 (was stale at #1–#73 + R1/R2). G0: ten v4 residual fixes (R1–R10) landed in `secure_multi_user_scaffolding_plan_v4.md`. G1: five ADRs drafted, awaiting human Accept. G2: `packages/secure_core/IMPLEMENTATION_MANIFEST.md` ships project layout, error envelope, fixture conventions, per-endpoint canonical recipe.)**
