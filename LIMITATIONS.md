...
| Login + CSRF cookie issuance | `packages/secure_core/src/auth/loginService.ts` + `/auth/login` route | Real scaffold | `POST /auth/login` mints `secure_session` (HttpOnly) and `csrf_token` (non-HttpOnly) cookies with constant-time anti-enumeration; `POST /auth/logout` revokes the session and clears both cookies idempotently. `password-reset/consume` and `email-verify/consume` bridge into a fresh session via `LoginService.mintSessionForUser` (aal2 for password reset, aal1 for email verify). Deployments inject their own `verifyPasswordHash` / `fetchPasswordHash` seam (Argon2id is the recommendation). |
