...
**Last updated: 2026-05-05 (DocsViewer fix — replaced broken Vite dynamic-import (`@vite-ignore` was making every page 404) with an iframe of the canonical `docs_site/` dev server at `http://localhost:3000/`; live probe + Pill state surface whether the docs server is reachable. Runs ↔ temp_runs/ merge — `GET /api/runs` surfaces script-driven example runs. Folder browser — `GET /api/browse` over the four workbench-managed roots + examples/. Examples gallery — `GET /api/examples` + `POST /api/examples/{name}/run` one-click runner.)**
...
| Docs viewer | `apps/workbench-ui/src/components/DocsViewer.tsx` + the docs_site dev server | Real | Iframes the canonical `docs_site/` Vite dev server (default `http://localhost:3000/`) so pages render from the same source the docs site itself serves. Periodic probe (every 5s) surfaces server-up/down state as a Pill; when down, a clear "run `scripts/docs/dev.sh`" hint replaces the iframe. Override the URL with `VITE_DOCS_BASE_URL` if the docs server runs on a non-default port. AGENTS.md "no inlined doc text" rule preserved — no doc body lives in the workbench UI bundle. |
