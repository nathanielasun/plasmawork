...
**Last updated: 2026-05-07 (Phase 0.5 Layers 1, 2, and 3 structurally complete with two rounds of post-Group-C audit fixes landed. Round-2 fixes (4 findings): live runsc probes detect binary presence + skip cleanly when absent (no spurious "not implemented" failures with `PLASMAWORK_RUNSC_PROBES=1` on dev hosts); archive-extraction quota accounting now reserves extracted bytes on top of the archive reservation and cleans up the `.extracted` dir on rejection; new regression tests pin SandboxRunner launch-before-running ordering and worker-upload FK + declared-size cap + archive-rejection-cleanup behavior; stale "stub" docs updated. Layer-3 surface: audit chain DB writer + verifier + WORM anchor committer (S3 Object Lock COMPLIANCE per ADR-0010), approval system with v4 §16.4 atomic consumption + §16.3 token_context_hash, capsule version + lock with VERSION_CONFLICT semantics, quota counters + §21.3 expiration sweep, run state machine, sandbox runner abstraction with `--network=none` default + forbidden-env-key strip + mount-source allowlist, worker token issuer (HMAC-signed envelopes with `requested_by_user_id` per v4 §18.1), worker artifact upload route (ADR-0012 server-derived path + streaming declared-size cap + reservation + archive validation through L2.11 with extracted-bytes quota accounting), SSRF-safe URL guard / fetcher (DNS-rebinding-resistant via per-request undici dispatcher) / webhook signer (v4 §26). v4 §29 spec-level invariants ship in `test/security/sandbox.test.ts` (7 always-on tests covering #38–#43 + #67); live-runtime gVisor probes are `it.todo` markers env-gated on `PLASMAWORK_RUNSC_PROBES=1` AND runsc-binary-on-PATH. secure_core suite: 477 tests pass + 87 skipped across 39 files. Convention checker at 1062 checks. Live-runtime gVisor CI lane (with real runsc), Layer-4 route plugins, and the FastAPI cut-over remain future work.)**
