...
**Last updated: 2026-05-05 (Sidebar collapsible — `<` button in the sidebar header collapses the nav to a 56px short-label rail; persisted across reloads via localStorage. DocsViewer cleanup — stripped explanatory chrome (no hero, no count pill, no architecture exposition); now just a horizontal nav of pages + the page body. DocsViewer consolidation — collapsed the docs into the workbench UI bundle via Vite's `import.meta.glob`. No iframe, no probe, no second server. Runs ↔ temp_runs/ merge — `GET /api/runs` surfaces script-driven example runs. Folder browser — `GET /api/browse` over the four workbench-managed roots + examples/. Examples gallery — `GET /api/examples` + `POST /api/examples/{name}/run` one-click runner.)**
