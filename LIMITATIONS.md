...
**Last updated: 2026-05-05 (DocsViewer consolidation — collapsed the docs into the workbench UI bundle via Vite's `import.meta.glob`. No iframe, no probe, no second server. The `docs_site/` Vite app is now optional — only needed to build a standalone hosted docs artifact. Runs ↔ temp_runs/ merge — `GET /api/runs` surfaces script-driven example runs. Folder browser — `GET /api/browse` over the four workbench-managed roots + examples/. Examples gallery — `GET /api/examples` + `POST /api/examples/{name}/run` one-click runner.)**
