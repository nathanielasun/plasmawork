...
**Last updated: 2026-05-06 (Phase 0.5 Layer-0 gate CLOSED — all five Layer-0 ADRs accepted by the human owner. ADR-0008 (TypeScript+Fastify+Drizzle+Postgres in `packages/secure_core/`, Shape A), ADR-0009 (gVisor + UNIX-domain-socket egress proxy), ADR-0010 (S3 Object Lock Compliance Mode + MinIO dev mock), ADR-0011 (AWS Secrets Manager + gitignored dev fallback), ADR-0012 (server-mediated worker uploads via `POST /api/workers/uploads`) are now load-bearing architectural commitments. The convention checker enforces `Status: Accepted` as an invariant going forward — backsliding fails the gate. Layer-1 task assignment is unblocked.)**
