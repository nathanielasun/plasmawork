...
**Last updated: 2026-05-05 (Runs ↔ temp_runs/ merge — `GET /api/runs` now surfaces script-driven example runs by walking `temp_runs/<id>/summary.json`; Diagnostics tab no longer silently drops ising / molecular_dynamics / laser_species / pde_wave runs. Folder browser — `GET /api/browse` over the four workbench-managed roots + examples/, with `..`/symlink-escape refusal; new `<FolderBrowser>` UI primitive replaces hand-typed paths in AutonomyPanel + RunControls. Examples gallery — UI gained `GET /api/examples` discovery + `POST /api/examples/{name}/run` one-click runner; sidebar nav defaults to it.)**
