...
**Last updated: 2026-05-06 (Phase 0.5 Layer-1 — six of seven Layer-1 tasks shipped in parallel. L1.1 centralized constants + L1.2 JCS canonicalization (`@truestamp/canonify`) + L1.3 token/HMAC utilities (Node `crypto` only, base64url, constant-time) + L1.4 error shape contract (23 codes, typed subclasses, dev/prod-aware mapper) + L1.6 secrets client (allowlisted, in-memory TTL cache, gitignored local dev provider, AWS provider stubbed pending L1.6-aws) + L1.7 audit logger (DI'd writer, redaction allowlist, canonical hash chain wiring through L1.2) + L1.8 schema migrations (28 tables, four DB roles, four migration files, V4-R3/R6/R7 fixes encoded). Cross-task integration verified: L1.7 hashes through L1.2's `canonicalize()`, L1.8's `schema.ts` exports stable for L1.7's audit row insert. Convention checker grew 771 → 816 checks. secure_core suite: 133 tests pass + 9 Postgres-gated tests skip cleanly. L1.5 (test fixtures) is the last Layer-1 task; it depends on L1.8's schema and is the natural foreground next step. Layer-0 gate (ADR-0008..ADR-0012) remains Accepted invariant.)**
