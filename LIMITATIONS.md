...
**Last updated: 2026-05-07 (Phase 0.5 Layers 1, 2, and 3 are now structurally complete: Layer-0 ADRs accepted; Layer-1 primitives shipped; Layer-2 middleware all 12 modules including L2.9 requireApprovalIfHighRisk; Layer-3 audit chain DB writer + verifier + WORM anchor committer (S3 Object Lock COMPLIANCE per ADR-0010), approval system with v4 §16.4 atomic consumption + §16.3 token_context_hash, capsule version + lock with VERSION_CONFLICT semantics, quota counters + §21.3 expiration sweep, run state machine, sandbox runner abstraction with `--network=none` default + forbidden-env-key strip, worker token issuer (HMAC-signed envelopes per v4 §18.1), worker artifact upload route (ADR-0012 server-derived path + streaming size cap + reservation), SSRF-safe URL guard / fetcher / webhook signer (v4 §26). secure_core suite: 456 tests pass + 81 Postgres-gated skip cleanly across 36 files. Convention checker at 1051 checks. Layer-4 route plugins, Layer-5 §29 integration suite, and the FastAPI cut-over remain future work.)**
