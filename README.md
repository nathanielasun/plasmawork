...
> **Status: Phase 6 — Sandboxed Agentic Code Generation complete (2026-05-03).** All five workstreams 6A–6E shipped: `simworkbench.codegen.CodeGenerator` deterministically renders a runnable Python `experiment.py`, `config.yaml`, `diagnostics.py`, `run.py`, README, and a `tests/` tree (unit / dimensional / smoke / regression / convergence-when-applicable) into `<capsule>/src/generated/` from any ModelSpec; `simworkbench.codegen.sandbox.sandboxed_write` is the single producer-side gate and refuses every write under `src/user_edits/`, `paper_sources/`, and `provenance/` regardless of caller intent (no `allow_user_edits_overwrite` knob exists at the library OR API layer); `simworkbench.codegen.TestGenerator` emits each plan-named pytest category as a real file with at least one `def test_…` body; `simworkbench.codegen.ValidationRunner` runs the freshly-generated experiment on the Phase-1 `Runner` (scipy-vetted LSODA, never a hand-rolled timestep loop per plan §15.2), writes per-diagnostic CSV under `validation/plots/`, a machine-readable `validation/status.yaml`, and a Markdown `validation/validation_summary.md` covering every §6E bullet; new "Generated Code" UI tab over four new backend endpoints (`GET /api/capsules/{name}/codegen`, `POST /api/capsules/{name}/codegen`, `GET /api/capsules/{name}/codegen/diff`, `POST /api/capsules/{name}/validate-run`) with the user-edits guard rendered as a separate tree (never co-mingled with the generated tree). `tests/integration/test_phase_6_gate_walk.py` written BEFORE implementation (per the ninth Phase Gate Procedure check) covers every gate verb: generate / run / review / edit / export, plus the API hard-rule guard against the bypass-flag pattern. Phases 0–5 remain complete.
