...
> **Status: Phase 9 — Parameter Sweeps, Optimization, and Uncertainty complete (2026-05-04).** All four workstreams 9A–9D shipped: `simworkbench.sweep` ships `SweepEngine` + `GridSampler` / `RandomSampler` / `LatinHypercubeSampler` / `AdaptiveSampler` ABC, with `SweepSpec.max_evaluations` as the hard cap (no bypass kwargs; signature-introspection regression locks the contract) and a JSON checkpoint format that survives kill-and-resume across multiple sessions. `simworkbench.optimization` exposes the `Optimizer` ABC, a deterministic `RandomSearchOptimizer` and a `BayesianOptimizerHook` that surfaces a structured `BayesianUnavailable` error when scikit-optimize is missing — `OptimizationProblem` carries a hard `budget`, `early_stop_threshold`, multi-objective `scalarization_weights`, and a constraint callback whose rejections count against the budget. `simworkbench.uncertainty` ships `MonteCarloPropagator`, `SensitivityAnalysis`, `ParameterDistribution` (normal / uniform / lognormal), `bootstrap_confidence_interval`, and `dominant_uncertainty`; `tests/validation/test_uq_calibration.py` pins linear / quadratic / mixed-distribution propagation against closed-form answers. `simworkbench.reports.ComparisonReport` ranks a `SweepReport` by chosen metric and writes `manifest.json` + `report.md` under the capsule; the new "Comparisons" UI tab (`ComparisonReportPanel.tsx`) reads `GET /api/comparison/{capsule}` and renders the table. `examples/parameter_sweep_quadratic/run_sweep.py` exercises the full sweep → checkpoint → comparison-report pipeline end-to-end. `tests/integration/test_phase_9_gate_walk.py` written BEFORE implementation; 19 parametric gate-walk tests cover every gate verb (sweep / rank / quantify / report / resume / provenance-chain / budget-cap / optimization). Phases 0–8 remain complete.
