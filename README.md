...
> **Status: Phase 4 — Agent-Assisted Paper Ingestion complete (2026-05-02; PDF + extracted-artifacts fixes 2026-05-03).** All five workstreams 4A–4E shipped, plus the post-close audit fixes: `simworkbench.ingestion.PaperImporter` orchestrates copy → text/tables/figures extraction → equations → parameters → interpretation generation, with **PDF support via `pypdf`** (a hard dep). Each ingestion produces `extracted_text.md`, `extracted_tables.json`, `extracted_figures.json`, `extracted_equations.json`, `extracted_parameters.yaml`, plus the four interpretation Markdown drafts under `paper_sources/`. `RegexEquationExtractor`, `RegexParameterExtractor` (flags `missing_units` rows, never fabricates units per plan §22), `TemplateInterpretationAgent` (every artifact marked "needs human review" per plan §Phase 4 hard rule). New "Papers" UI tab (PaperReview / EquationList / ParameterList / **editable** InterpretationView) wired to three new backend endpoints (`POST /api/papers/import`, `GET /api/papers/{capsule}/extracted`, `POST /api/papers/{capsule}/edit`) — every edit Pydantic-validated AND boundary-checked (empty/whitespace reviewer rejected at the API), then appended to `provenance/agent_trace.md`. `tests/integration/test_phase_4_gate_walk.py` exercises every plan-named gate verb end-to-end, including a real PDF success path. Phases 0–3 remain complete.
