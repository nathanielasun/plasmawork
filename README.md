...
> **Status: Phase 5 — ModelSpec Generation and Module Mapping complete (2026-05-03).** All four workstreams 5A–5D shipped: `simworkbench.modeling.ModelSpecGenerator` transforms reviewed Phase-4 interpretation artifacts into a schema-valid ModelSpec under `<capsule>/model/model_spec.yaml` (with a structured-error refusal when interpretation rows lack `edited_by`, carrying plan §Phase 4's hard rule into Phase 5 input gating); `ModuleMatcher` walks `packages/physics_modules/` and produces a structured match report with per-bullet sub-scores (domain, I/O, units, solver compatibility); `GapAnalyzer` covers all five plan §10.4 categories (missing modules / missing data / unsupported regimes / invalid solver choices / validation gaps); `ExperimentProposer` writes `<capsule>/experiment_proposal.md` with all five §Phase 5 / 5D bullets (minimal sim, fidelity extensions, computational cost, validation path, backend recommendation). New "Proposals" UI tab over one new backend endpoint (`POST /api/proposals` runs the full pipeline). `tests/integration/test_phase_5_gate_walk.py` written BEFORE implementation (per the ninth Phase Gate Procedure check) covers every gate verb: transform / map / analyze / propose, plus the hard-rule guard. Phases 0–4 remain complete.
