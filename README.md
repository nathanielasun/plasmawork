...
> **Status: Phase 8 — HPC and Hardware Backends complete (2026-05-04).** All six workstreams 8A–8F shipped: `simworkbench.runtime.solver_backend` exposes the formal `SolverBackend` ABC + `BackendCapabilities` descriptor; `simworkbench.backends` ships `BackendRegistry`, `BackendStatus` lifecycle gated inside the library by single-use approval tokens (`grant_backend_approval`), Pydantic-validated metadata that refuses malformed entries, and capability-aware `recommend(spec)`. `python_cpu` and `numba_cpu` are now `validated` (cross-backend agreement within 1e-6 relative on the canonical 2-species conversion experiment); `numba_cpu` JITs the rate-equation RHS through Numba and falls back to a NumPy implementation when Numba is missing so the success path is always reachable. The C++ kernel pipeline (`packages/solver_backends/cpp/`) ships a CMake build + ctypes ABI wrapper that compiles a reference `axpy` kernel; Fortran skeleton mirrors the contract. The CUDA adapter (`packages/solver_backends/cuda/`) exposes a non-raising capability probe + memory estimator + determinism warning; `ADR-0006-determinism-policy.md` documents the workbench's GPU determinism stance and the `provenance.lock` writer reads each backend's `CAPABILITIES.deterministic` at save time per the policy. HPC orchestration (`simworkbench.hpc`) ships `SlurmJob` (generates a self-contained batch bundle), `RayAdapter` (optional Ray cluster path), and `import_remote_result` (reconstructs a `RunResult` from a remote run); `scripts/dev/submit_slurm.sh` and `scripts/dev/import_hpc_result.sh` are the CLI entry points. The external-PIC adapter contract (`ExternalSimulatorAdapter` ABC) is in place with a reference `StubPICAdapter`. `tests/integration/test_phase_8_gate_walk.py` written BEFORE implementation; 15 gate-walk tests cover every gate verb (run-locally / run-remotely / same-interface / capability-detect / determinism-marked / lifecycle-gate). Phases 0–7 remain complete.
