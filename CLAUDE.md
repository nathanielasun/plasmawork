...
Current state:
- Default convention checker covers every Phase 0/1/2 entity (290 checks); the opt-in `--include-open-workstreams` mode is empty awaiting Phase 3.
- Phase status synchronized across `README.md`, `program_development/milestones/{phase_00,phase_01,phase_02}_*.md`, `program_development/timeline.md`, all `docs_site/src/content/*.tsx` pages that name the phase, and this file.
...
Phase 3 is next per plan §Phase 3. Open it via **Phase Gate Procedure → Starting a workstream** below: enumerate plan deliverables, add per-entity opt-in convention-checker assertions, then implement until the checker is green.
