...
**Phase 3** (Internal Tool SDK and Registry) — complete 2026-05-02. All five workstreams 3A–3E shipped. `BaseTool` ABC + `ToolInput`/`ToolOutput` contracts at `simworkbench.tools` (plan §9.4); `ToolMetadata` Pydantic schema for `tool.yaml`; lifecycle state machine (`draft → candidate → validated → trusted → deprecated`) with agent/human gating per plan §9.5; `ToolRegistry` discovers `packages/internal_tools/registry/` and `local_cache/imported_tools/`; the reference `absorption_spectrum_diagnostic` example; seven category templates (`diagnostic`, `visualization`, `import_tool`, `physics_module`, `solver_adapter`, `validation`, `paper_extraction`); the Tools tab in the workbench UI (ToolList / ToolDetail / ToolDocs / ToolStatus) over four new backend endpoints (`GET /api/tools{,/<name>{,/docs}}`, `POST /api/tools/<name>/status`); `scripts/dev/refresh_registry.sh` rewrites `packages/internal_tools/registry/index.yaml` from the discovered tools.

Current state:
- Default convention checker covers every Phase 0/1/2/3 entity (358 checks); the opt-in `--include-open-workstreams` mode is empty awaiting Phase 4.
- Phase status synchronized across `README.md`, `program_development/milestones/{phase_00,phase_01,phase_02,phase_03}_*.md`, `program_development/timeline.md`, all `docs_site/src/content/*.tsx` pages that name the phase, and this file.
...
- `bugs_and_fixes/agent_error_patterns.md` now carries 24 named patterns. Read them before changing: convention-checker logic, `.gitignore`, milestone-naming, ModelSpec validators (flexible-dict fields), test wrappers, cached singletons, API factories, checkpoint guards, status-flip commits, the lint policy, plus the post-Phase-2 audit patterns (writers-without-producers, schema drift between writers and validators, destructive-before-guard in exporters, absolute paths in exports, source-tree compile artifacts, duplicated phase status).

Phase 4 is next per plan §Phase 4. Open it via **Phase Gate Procedure → Starting a workstream** below: enumerate plan deliverables, add per-entity opt-in convention-checker assertions, then implement until the checker is green.
