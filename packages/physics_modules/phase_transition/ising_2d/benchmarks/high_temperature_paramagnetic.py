"""Ising 2D — high-temperature paramagnetic regime."""
...
def run_benchmark() -> ValidationReport:
    ...
    return ValidationReport(
        name="ising_high_temperature_paramagnetic",
        passed=passed,
        metric=abs(m),
        tolerance=0.3,
        detail=f"|m|={abs(m):.4f} (expect < 0.3 at T*=4.0)",
        metadata={"reference": "Ising 2D paramagnetic phase"},
    )
