...
## Error Pattern: Protected route uses framework body schema instead of audit-aware validation

### Why it is bad
Fastify `schema.body` can reject malformed input, but it does not run the
repository's recursive forbidden-field scan or emit the
`request.unexpected_field` audit event. A route can appear validated while
still accepting server-derived fields on nearby endpoints, or while
returning inconsistent envelopes in production.

### Required behavior
- Protected JSON-body routes use `validateInputSchema` / `bodyValidation`
  in the middleware chain.
- Body validation runs before handler logic and before any service call.
- Fastify schemas are acceptable for query/path validation, but body
  security is owned by the audit-aware validator.

### Detection
- Grep protected routes for `schema: { body:`. Each match needs a clear
  explanation or must be converted to `bodyValidation`.
- Negative route tests send forbidden fields (`status`, `storage_path`,
  `workspace_id`, `*_hash`) and assert a 400 with no service call.

### Bug log
- 2026-05-07 *Secure-core Layer-4 route and worker hardening*: multiple
  Layer-4 route bodies relied on Fastify schemas instead of
  `validateInputSchema`. Fix: shared route body-validation helper and
  updated tests across L4 routes.

---

## Error Pattern: Route accepts storage facts that should be derived from a workspace object

### Why it is bad
Fields like `content_hash`, `storage_path`, lifecycle `status`, and
workspace ids are facts about server-controlled objects. If an HTTP route
accepts them from the client, a caller can smuggle a path/hash/status
that did not come from the workspace-scoped artifact table. The route may
still pass authorization because the object lookup never happened.

### Required behavior
- Accept object references such as `source_artifact_id`.
- Resolve content hash, storage path, current status, and workspace
  ownership server-side after membership/capability checks.
- Lifecycle status changes go through dedicated approval/lifecycle
  endpoints, not general PATCH bodies.

### Detection
- Grep route body interfaces and JSON schemas for `content_hash`,
  `storage_path`, `workspace_id`, or `status`.
- Route tests for create/update send those fields and assert rejection
  before the service is called.

### Bug log
- 2026-05-07 *Secure-core Layer-4 route and worker hardening*: capsule
  and tool routes accepted `content_hash` / `storage_path`; tool PATCH
  accepted `status`. Fix: routes accept `source_artifact_id`, resolve
  storage facts server-side, and reject status in PATCH.

---

## Error Pattern: High-risk action checks approval in the route but not at commit

### Why it is bad
Approval middleware proves a token was valid at pre-handler time. A
membership, role, or lifecycle mutation can still race with revocation or
role removal before the SQL write commits. If the service trusts only the
earlier route check, the final state can be changed by an actor who no
longer has the required capability.

### Required behavior
- High-risk routes consume an L2.9 approval token before handler side
  effects.
- Services that mutate privilege-bearing state re-check live membership
  and capability inside the transaction immediately before commit.
- Regression tests cover both route-level missing approval and
  service-level commit-time denial.

### Detection
- For each high-risk route, locate both the L2.9 middleware in the route
  and a transaction-local capability check in the service.
- Grep services for membership/lifecycle writes that do not query
  `workspace_memberships` / role permissions inside the transaction.

### Bug log
- 2026-05-07 *Secure-core Layer-4 route and worker hardening*: workspace
  membership mutations lacked approval wiring and commit-time capability
  rechecks. Fix: approval request ids route through L2.9; service
  transactions re-check `workspace:manage_members` before mutating.

---

## Error Pattern: Derived artifact accounting reserves but does not commit every byproduct

### Why it is bad
Uploads and sandbox jobs can write one artifact, then derive additional
files from it (archive extraction, generated logs, transformed outputs).
If quota is reserved for a byproduct but only the primary artifact's
reservation is committed, the expiration sweep returns quota for bytes
that remain on disk. If the byproduct is never reserved, a compressed
archive can expand past quota invisibly.

### Required behavior
- Track the complete creation set for a write path: primary file,
  extracted/generated byproducts, metadata sidecars.
- Reserve and commit quota for every persisted byproduct.
- On any validation, reservation, or commit failure, remove every file
  from the creation set and release uncommitted reservations.

### Detection
- Grep writer paths for multiple filesystem writes or extraction steps;
  count reservations and commits.
- Regression tests assert both the number of `reserveBytes` calls and
  the number of `commitReservation` calls for clean derived-output paths.

### Bug log
- 2026-05-07 *Secure-core Layer-4 route and worker hardening*: archive
  upload success reserved extracted bytes but did not commit that
  reservation. Fix: commit both archive and extracted reservations and
  expose `extracted_bytes` / summed audit metadata.

---

## Error Pattern: Security-sensitive placeholder returns a success-shaped result

### Why it is bad
A placeholder that returns `{ ok: true }`, a 200 response, or an
audit row marked `succeeded` teaches operators that the security action
happened. For destructive remediation, sandboxing, approval, or audit
anchoring, a no-op success is worse than a visible failure because it
can drive real incident response decisions from false state.

### Required behavior
- Security-sensitive unimplemented actions fail closed.
- Attempts may be audited, but the audit result is `failed` or `denied`
  and metadata marks `not_implemented`.
- The handler throws a typed error until the real side effect ships.

### Detection
- Grep security code for `TODO`, `not implemented`, `placeholder`, and
  success-shaped return objects.
- Tests call the placeholder path and assert a typed failure plus a
  failed/denied audit row.

### Bug log
- 2026-05-07 *Secure-core Layer-4 route and worker hardening*:
  operator remediation returned success without implementing
  `delete_session`, `revoke_membership`, or `lock_capsule`. Fix:
  record the attempt as failed and throw `INTERNAL_ERROR` with
