...
Gateway proxy routes that forward to legacy FastAPI handlers must keep an explicit capability map. Every state-changing proxied route is either mapped to required workspace/platform capabilities or refused before forwarding. UI visibility is not an authorization boundary. Platform capabilities are derived from server-side membership/role records across the relevant platform scope, not from the active workspace role list and never from request bodies.
...
An environment flag is not a sandbox. Any production/gateway-required code-execution preview path must run through a configured sandbox command/runtime or fail closed.
