...
Existence checks (steps 1–6 above) are necessary but not sufficient. The convention checker proves files exist. It does not prove that the phase gate's *behaviors* work. Phases 1, 2, 3, and 4 each shipped an incomplete close (Phase 4 twice) because the agent confused "all entities exist" or "the gate verb is satisfied" with "the plan's deliverable list is met". Add these twelve behavioral checks before any close commit:
...
The named patterns each of these defends against live in `bugs_and_fixes/agent_error_patterns.md` (33 patterns; the latest four are post-Phase-4: skipping-task-bullets, multi-target-verbs-partially-implemented, validating-at-ui-not-api, structured-error-without-success-path).
