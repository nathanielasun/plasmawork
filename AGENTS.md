...
High-risk actions must carry an approval request id and consume the approval token through L2.9 before handler side effects. Services that mutate membership, roles, lifecycle status, or other privilege-bearing state must re-check the actor's live capability inside the transaction immediately before commit.
